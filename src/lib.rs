...
impl TreRegex {
    ...
    pub fn new(pattern: String, ignore_case: Option<bool>) -> Result<Self> {
        let c_pattern = CString::new(pattern)
            .map_err(|_| Error::from_reason("Pattern contains null bytes"))?;
        ...
        Ok(Self { preg: Box::into_raw(preg) })
    }
    ...
    fn build_params(&self, opts: &Option<TreRegexOptions>) -> tre_regaparams_t {
        ...
        if let Some(o) = opts {
            if let Some(e) = o.max_errors { params.max_err = e as c_int; }
            if let Some(i) = o.max_insertions { params.max_ins = i as c_int; } else if o.max_errors.is_some() { params.max_ins = 0; }
            if let Some(d) = o.max_deletions { params.max_del = d as c_int; } else if o.max_errors.is_some() { params.max_del = 0; }
            if let Some(s) = o.max_substitutions { params.max_subst = s as c_int; } else if o.max_errors.is_some() { params.max_subst = 0; }
            if let Some(c) = o.max_cost { params.max_cost = c as c_int; }

            if let Some(w) = o.weight_insertion { params.cost_ins = w as c_int; }
            if let Some(w) = o.weight_deletion { params.cost_del = w as c_int; }
            if let Some(w) = o.weight_substitution { params.cost_subst = w as c_int; }
