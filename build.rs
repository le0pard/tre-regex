...
fn main() {
    ...
    // 1. Automatically Download, Verify, and Extract TRE
    if !Path::new(&tre_dir).exists() {
        println!("cargo:warning=Downloading TRE source...");
        let url = format!("https://github.com/laurikari/tre/archive/{}.tar.gz", tre_version);
        ...
        file.read_to_end(&mut buffer).expect("Failed to read tarball");
...
#include <alloca.h>
    ...
    build.file(format!("{}/lib/tre-ast.c", tre_dir))
